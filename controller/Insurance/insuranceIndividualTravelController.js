...
const insuranceTravelController = {
  ...
  async deleteIndividualTravel(req, res, next) {
    try {
      ...
      const existingInsurance = await IndividualTravel.findById(insuranceTravelId);
      ...
      await IndividualTravel.findByIdAndDelete(insuranceTravelId);
  ...
  async getAllIndividualTravel(req, res, next) {
    try {
      ...
      const totalInsurance = await IndividualTravel.countDocuments(query); // Get the total number of insurances for the user
      ...
      const insurances = await IndividualTravel.find(query)
        .skip(skip)
        .sort({ createdAt: -1 })
